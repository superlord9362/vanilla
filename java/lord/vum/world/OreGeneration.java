...
public class OreGeneration implements IWorldGenerator {
	...
	public void generate(Random random, int chunkX, int chunkZ, World world, IChunkGenerator chunkGenerator, IChunkProvider chunkProvider) {
		if(!(chunkGenerator instanceof ChunkGeneratorHell) && !(chunkGenerator instanceof ChunkGeneratorEnd)) {
			BlockPos pos;
			for(int n=0; n<Configs.worldgen.jadeVeinFrequency; n++){
			...
			for(int n=0; n<Configs.worldgen.amethystVeinFrequency; n++){
				pos = new BlockPos(chunkX*16+random.nextInt(16), random.nextInt(ORE_MAX-ORE_MIN)+ORE_MIN, chunkZ*16+random.nextInt(16));
			...
			for(int n=0; n<Configs.worldgen.marbleVeinFrequency; n++){
				...
				new WorldGenMinable(BlockInit.MARBLE.getDefaultState(), VEIN_SIZE_LIME, BlockMatcher.forBlock(Blocks.STONE)).generate(world, random, pos);
			}
			
			for(int n=0; n<Configs.worldgen.limestoneVeinFrequency; n++){
				pos = new BlockPos(chunkX*16+random.nextInt(16), random.nextInt(ORE_LIME_MAX-ORE_LIME_MIN)+ORE_LIME_MIN, chunkZ*16+random.nextInt(16));
				new WorldGenMinable(BlockInit.LIMESTONE.getDefaultState(), VEIN_SIZE_LIME, BlockMatcher.forBlock(Blocks.STONE)).generate(world, random, pos);
			}
