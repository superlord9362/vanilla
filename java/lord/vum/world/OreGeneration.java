...
public class OreGeneration implements IWorldGenerator {
	...
	public void generate(Random random, int chunkX, int chunkZ, World world, IChunkGenerator chunkGenerator, IChunkProvider chunkProvider) {
		if(!(chunkGenerator instanceof ChunkGeneratorHell) && !(chunkGenerator instanceof ChunkGeneratorEnd)) {
			for(int n=0; n<VEIN_FREQ_ORES; n++){
			...
			for(int n=0; n<VEIN_FREQ_DECO; n++){
				...
				pos2 = new BlockPos(chunkX*16+random.nextInt(16), random.nextInt(ORE_LIME_MAX-ORE_LIME_MIN)+ORE_LIME_MIN, chunkZ*16+random.nextInt(16));
